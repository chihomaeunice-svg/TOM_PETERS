...
export default function SellerRegister() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      await registerSeller(email, password, displayName, businessName);
  ...
  return (
    <div className="min-h-screen bg-tp-cream flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          <Link to="/" className="font-display text-3xl tracking-[0.15em] text-tp-charcoal uppercase">
            TOM PETERS
          </Link>
          <p className="text-sm text-tp-taupe mt-3 tracking-wider">Seller Account Registration</p>
        </div>

        <div className="bg-tp-gold/10 border border-tp-gold/30 rounded px-4 py-3 mb-6 text-sm text-tp-charcoal">
          Your account will be reviewed by our team before activation. You'll be able to log in once approved.
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-tp-border rounded p-8 shadow-luxe space-y-5">
          ...
          {[
            ...
            { label: 'Business Name', type: 'text', value: businessName, set: setBusinessName, placeholder: 'Your Brand LLC' },
            { label: 'Email', type: 'email', value: email, set: setEmail, placeholder: 'you@brand.com' },
          ...
          <p className="text-center text-sm text-tp-taupe">
            Not sure yet?{' '}
            <Link to="/become-a-seller" className="text-tp-gold hover:underline">Submit an inquiry first</Link>
          </p>
        </form>
      </div>
    </div>
